...
class Web3Helper {
  ...
  async ExecuteMethod(
    proceedAs = { walletId: null, address: null, pincode: null },
    methodType,
    contract,
    funcName,
    params = []
  ...
  async Send(proceedAs, contract, funcName, params = []) {
    try {
      ...
      if (!this.nonMetaFunctions?.[contract].includes(funcName)) {
        ...
        // TODO
        throw new Error(
          'Venly integration for executing functions as non-meta tx is unimplemented.'
        );
      }
      return result;
    } catch (error) {
  ...
  strToERC20(str) {
    ...
    const result = this.web3.utils.toBN(
      beforeDecimalVal.concat(afterDecimalVal)
    );
    return result;
  }
